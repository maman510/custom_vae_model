...
class Encoder(keras.Model):
    ...
    def __init__(self, latent_dims, input_dims):
        ...
        self.filters = self._get_encoder_filters(self.input_dims)


    def build(self, input_shape, hp):
     
        self.downsample_layers = []
        kernel_size = hp.Choice('encoder_conv_kernel', [3, 5])
        for i in range(len(self.filters)):
            self.downsample_layers.append(keras.layers.Conv2D(self.filters[i], 
                                                              kernel_size=(kernel_size,kernel_size), 
                                                              activation=hp.Choice('encoder_conv_activation',["relu", "leaky_relu"]), 
                                                              padding="same", 
                                                              name=f"encoder_conv_{i+1}"))
            
            self.downsample_layers.append(keras.layers.BatchNormalization(name=f"encoder_batch_norm_{i+1}"))
    ...
    def summary(self):
        ...
        self.summary_table = tool_box.color_string('yellow', tabulate(table, headers, tablefmt="fancy_grid"))
        print("\n\n\t")
